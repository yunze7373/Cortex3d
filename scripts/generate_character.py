...
环境变量:
  ...
  ZIMAGE_URL        - 本地Z-Image服务地址 (local模式)
    ...
    proc_group.add_argument(
        ...
        default="birefnet-general",
        metavar="MODEL",
        help="后处理(切割后)的背景去除模型。推荐选择: birefnet-general(最新,效果最好), isnet-general-use(保守,保留道具), u2net(经典) (默认: birefnet-general)"
    )
...
=================================================================
    ...
    # 调用生成器 (proxy/direct 模式)
        ...
        result = generate_character_multiview(
            ...
            rembg_model=getattr(args, 'rembg_model', 'birefnet-general')
        )
    elif args.mode == "direct":
