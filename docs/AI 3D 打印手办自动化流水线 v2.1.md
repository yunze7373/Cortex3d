...
版本: 2.1
核心引擎: Google Gemini (NanoBanana Pro) + InstantMesh (备选 SF3D/Dust3R) + Blender Automation
...
graph TD
    ...
    subgraph S1[Stage 1: 标准化多视图采集]
        Orchestrator --> GeminiAPI[Google Gemini API (NanoBanana Pro)]
    ...
    subgraph S2[Stage 2: 基础几何构建]
        ViewsClean --> AlgoSelect{算法选择}
        AlgoSelect -->|均衡/快速| IMesh[主力: InstantMesh]
        AlgoSelect -->|高材质/纹理| SF3D[进阶: SF3D]
        AlgoSelect -->|非标准视图| Dust3R[进阶: Dust3R]
        
        IMesh & SF3D & Dust3R --> MeshCheck{3D 裁判<br/>(Trimesh 水密/流形检查)}
...
核心生成: Google Gemini API (指定模型: NanoBanana Pro / Imagen 3)

优势: 这是目前 Gemini 体系中最强的图像生成模型，具备极高的指令遵循能力和纹理细节表现，能最大程度保证四视图的一致性和解剖学正确性。
...
在此阶段，我们采用“一主两备”的策略，根据具体需求选择重建算法：

主力模型: InstantMesh

定位: 均衡型 (Balanced)。

特点: 速度快，几何与纹理平衡较好，部署相对简单。适合作为默认选项。

进阶备选 A: SF3D (Stable Fast 3D)

定位: 高材质型 (Material-Focused)。

优势: 如果输入图像包含复杂的金属反光或特殊材质，SF3D 对光照和材质的还原度通常优于 InstantMesh，生成的几何底子也更干净。

适用: 对表面质感要求极高的角色。

进阶备选 B: Dust3R (Dense Unconstrained Stereo)

定位: 高鲁棒型 (Robust Geometry)。

优势: 不依赖相机参数，纯粹通过像素匹配重建点云。当 Stage 1 生成的四视图透视稍微对不齐（例如 Gemini 画歪了）时，InstantMesh 可能会崩，但 Dust3R 往往能硬算出来。

注意: 需要额外的点云转网格步骤 (Point-to-Mesh)。

兜底模型: TripoSR

定位: 安全网 (Safety Net)。

触发条件: 当上述高级模型生成失败（非流形/破损）时自动触发。
...
四、 提示词工程指南 (针对 Gemini NanoBanana Pro 优化)
...
$$$$
 ...
 申请 Google Gemini API Key (确保有 NanoBanana Pro 访问权限)。
...
$$$$
 ...
 部署 InstantMesh、SF3D、Dust3R 的本地推理环境。
...
$$$$
 ...
 reconstructor.py: 编写调用 InstantMesh/SF3D/Dust3R 的子进程代码，实现算法选择逻辑。
