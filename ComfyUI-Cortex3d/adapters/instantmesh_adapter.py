...
_fb = FileBridge()
_cache = ResultCache("instantmesh")
...
class InstantMeshAdapter:
    ...
    def reconstruct(
        ...
        # ── 缓存查找 ──
        cache_kw = dict(image_path=image_path, diffusion_steps=diffusion_steps,
                        guidance_scale=guidance_scale, seed=seed,
                        texture_resolution=texture_resolution, export_texture=export_texture)
        hit = _cache.get(**cache_kw)
        if hit:
            return CortexMesh(file_path=hit, format="obj", source_algo="instantmesh")
